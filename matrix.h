    ...
    for (size_t k = 0; k < A.size().second; ++k)
    {
        ...
        if (op)
        {
            if (1/(A[cnt-1][k]) != 1 && show) {
                std::cout << "d(" << cnt << ", " << Rational(1) / A[cnt-1][k] << ")" << '\n';
                A.d_ERO(cnt - 1, Rational(1) / A[cnt-1][k]);
                std::cout << A << '\n' << '\n';
            }
            else
            {
            ...
            for (size_t i = cnt; i < A.size().first; ++i)
            {
                if (A[i][k] != 0 && show)
                {
                    std::cout << "l(" << i+1 << ", " << cnt << ", " << -A[i][k] << ")" << '\n';
                    A.l_ERO(i, cnt - 1, -A[i][k]);
                    std::cout << A << '\n' << '\n';
                }
                else
                {
